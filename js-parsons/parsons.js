(function($, _) { // wrap in anonymous function to not show some helper variables
  ...
  LineBasedGrader.prototype._codelinesAsString = function() {
    var student_code = this.parson.getModifiedCode("#ul-" + this.parson.options.sortableId);
    var executableCode = "";
    $.each(student_code, function(index, item) {
      // split codeblocks on br elements
  ...
  function download(filename, text) {
    ...
    element.setAttribute('href', 'data:text/x-python;charset=utf-8,' + encodeURIComponent(text));
   ...
   ParsonsWidget.prototype.export = function(name) {
      var codeLines = this.grader._codelinesAsString();
      download(name, codeLines)
   }
